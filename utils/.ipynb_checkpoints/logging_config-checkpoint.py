...
def setup_logger(name, log_file, log_level='DEBUG'):
    ...
    if not logger.hasHandlers():
        ...
        logger.info('======== Start of Run: {timestamp} ======='.format(timestamp=datetime.now()))
    
    return logger

def finalize_logger(logger):
    """Logs the end time and closes all handlers for the logger."""
    logger.info('======== End of Run: {timestamp} ======='.format(timestamp=datetime.now()))
    for handler in logger.handlers[:]:  # Use a slice to avoid modifying the list during iteration
        handler.close()
