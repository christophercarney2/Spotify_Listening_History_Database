...
def get_db_connection():
    ...
    conn_string = (
        f"postgresql+psycopg://postgres:{postgresql_pw}@localhost:5432/spotify"
    )
...
def load_data_to_db(df, db, table_name, logger):
    ...
    with db.connect() as conn:
        ...
        if table_name in inspector.get_table_names():
            while True:
                user_input = input(
                    f"The table '{table_name}' already exists. Do you want to drop it and proceed? (yes/no): "
                )
...
def check_new_tracks_and_artists(
    ...
    for track_uri in batch_track_uris:
        ...
        track_artist_exists = conn.execute(
            select(track_artists_table).where(
                track_artists_table.c.spotify_track_uri == track_uri
            )
        ).scalar()
...
def insert_track_artist(
    conn, track_artists_table, spotify_track_uri, spotify_track_id, track_artist, logger
):
    """
    ...
    logger.info(
        f"Track artist: track {spotify_track_id}, artist {track_artist['name']} added"
    )
...
def insert_album(conn, albums_table, album, logger):
    ...
    try:
        ...
        release_date = datetime.strptime(
            album["release_date"], precision_format_map[album["release_date_precision"]]
        ).date()
        ...
        if isinstance(artist["genres"], list):
            for genre in artist["genres"]:
                genre_insert_stmt = insert(artist_genre_table).values(
                    spotify_artist_id=artist["id"], genre=genre
                )
...
def update_music_listening_history(
    ...
    mlh_update_stmt = (
        update(music_listening_history_table)
        .where(
            music_listening_history_table.c.spotify_track_uri
            == tracks_table.c.spotify_track_uri
        )
        .values(
...
def process_duplicate_artists(
    ...
    dupe_query = (
        select(artists_table.c.artist_name)
    ...
    query = select(
        ...
        artists_table.c.spotify_artist_id,
        artists_table.c.followers,
    ).where(artists_table.c.artist_name.in_(duplicate_artist_names))
    result = conn.execute(query)

    # Creates a DataFrame from the query result
    dupe_artist_df = pd.DataFrame(
        result, columns=["artist_name", "spotify_artist_id", "followers"]
    )
    ...
    for index, row in dupe_artist_df.iterrows():
        ...
        mlh_dupe_artist_stmt = (
            update(music_listening_history_table)
            .where(
                music_listening_history_table.c.spotify_artist_id == spotify_artist_id
            )
            .values(artist_name=artist_name)
        )
        artists_dupe_artist_stmt = (
            update(artists_table)
            .where(artists_table.c.spotify_artist_id == spotify_artist_id)
            .values(artist_name=artist_name)
        )
